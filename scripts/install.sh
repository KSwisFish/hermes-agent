...
# Configuration
REPO_URL_SSH="git@gitee.com:fish_zzc/hermes-agent.git"
REPO_URL_HTTPS="https://gitee.com/fish_zzc/hermes-agent.git"
HERMES_HOME="${HERMES_HOME:-$HOME/.lumina}"
...
#   code at /usr/local/lib/lumina-agent, command at /usr/local/bin/hermes,
#   data still at /root/.lumina (HERMES_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.lumina/lumina-agent"
            echo "                   default (root, Linux): /usr/local/lib/lumina-agent"
            echo "  --hermes-home PATH  Data directory (default: ~/.lumina, or \$HERMES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hermes installs the code under"
            echo "  /usr/local/lib/lumina-agent and links the command into"
            echo "  /usr/local/bin/hermes (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HERMES_HOME"
            echo "  (default /root/.lumina).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HERMES_HOME/lumina-agent are preserved in-place."
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HERMES_HOME/lumina-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HERMES_HOME/lumina-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/lumina-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HERMES_HOME/lumina-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$HERMES_HOME/lumina-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HERMES_HOME/lumina-agent/.git" ]; then
            INSTALL_DIR="$HERMES_HOME/lumina-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/lumina-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/lumina-agent"
...
install_node() {
    ...
    log_info "Extracting to ~/.lumina/node/..."
    ...
    # Place into ~/.lumina/node/ and symlink binaries into the same bin dir
    ...
    log_success "Node.js $installed_ver installed to ~/.lumina/node/"
    HAS_NODE=true
}
...
copy_config_templates() {
    ...
    # Create ~/.lumina directory structure (config at top level, code in subdir)
    mkdir -p "$HERMES_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.lumina/.env (top level, easy to find)
    if [ ! -f "$HERMES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HERMES_HOME/.env"
            log_success "Created ~/.lumina/.env from template"
        else
            touch "$HERMES_HOME/.env"
            log_success "Created ~/.lumina/.env"
        fi
    else
        log_info "~/.lumina/.env already exists, keeping it"
    fi
    ...
    # Create config.yaml at ~/.lumina/config.yaml (top level, easy to find)
    if [ ! -f "$HERMES_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HERMES_HOME/config.yaml"
            log_success "Created ~/.lumina/config.yaml from template"
        fi
    else
        log_info "~/.lumina/config.yaml already exists, keeping it"
    fi
...
SOUL_EOF
        log_success "Created ~/.lumina/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.lumina/"

    # Seed bundled skills into ~/.lumina/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        ...
        log_info "Syncing bundled skills to ~/.lumina/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.lumina/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HERMES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$HERMES_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.lumina/skills/"
            fi
        fi
    fi
}
...
maybe_start_gateway() {
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.lumina/logs/gateway.log"
