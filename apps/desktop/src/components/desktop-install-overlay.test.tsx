...
describe('DesktopInstallOverlay first-run setup', () => {
  it('shows the remote/local choice without installer progress', async () => {
    ...
    expect(await screen.findByText('Set up lumina')).toBeTruthy()
    expect(screen.getByText('Connect to existing lumina')).toBeTruthy()
    expect(screen.getByText('Install lumina locally')).toBeTruthy()
  ...
  it('continues local bootstrap only when Install Hermes locally is selected', async () => {
    ...
    fireEvent.click(await screen.findByText('Install lumina locally'))

    expect(desktop.continueBootstrapLocal).toHaveBeenCalledTimes(1)
    expect(screen.getByText('Set up lumina')).toBeTruthy()

    act(() => {
      desktop.emitBootstrapEvent({ type: 'manifest', protocolVersion: 1, stages: [] })
    })

    await waitFor(() => expect(screen.queryByText('Set up lumina')).toBeNull())
  ...
  it('surfaces a recoverable error when the local-bootstrap bridge is unavailable', async () => {
    ...
    const install = (await screen.findByText('Install lumina locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)

    expect(
      await screen.findByText('Local installation could not start. Restart lumina and try again.')
    ).toBeTruthy()
  ...
  it('keeps the local-start error when the first snapshot commits under the click', async () => {
    ...
    const install = (await whenPresent('Install lumina locally')).closest('button') as HTMLButtonElement
    fireEvent.click(install)
    ...
    expect(screen.queryByText('Local installation could not start. Restart lumina and try again.')).toBeTruthy()
  })
  ...
  it('clears a stale local-start error when a repair presents a different root', async () => {
    ...
    fireEvent.click((await screen.findByText('Install lumina locally')).closest('button') as HTMLButtonElement)
    expect(
      await screen.findByText('Local installation could not start. Restart lumina and try again.')
    ).toBeTruthy()
    ...
    expect(screen.queryByText('Local installation could not start. Restart lumina and try again.')).toBeNull()
  })
  ...
  it('opens the remote connection form from the first-run choice', async () => {
    ...
    fireEvent.click(await screen.findByText('Connect to existing lumina'))
  ...
  it('offers remote connection from the unsupported packaged install screen', async () => {
    ...
    expect(await screen.findByText('lumina needs a one-time install')).toBeTruthy()
    ...
    await waitFor(() => expect(screen.queryByText('Gateway URL')).toBeNull())
    expect(screen.queryByText('lumina needs a one-time install')).toBeNull()
  })
})
