...
class TestGetDefaultHermesRoot:
    ...
    def test_no_hermes_home_returns_native(self, tmp_path, monkeypatch):
        """When HERMES_HOME is not set, returns ~/.lumina."""
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_hermes_root() == tmp_path / ".lumina"

    def test_hermes_home_is_native(self, tmp_path, monkeypatch):
        """When HERMES_HOME = ~/.lumina, returns ~/.lumina."""
        native = tmp_path / ".lumina"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_hermes_home_is_profile(self, tmp_path, monkeypatch):
        """When HERMES_HOME is a profile under ~/.lumina, returns ~/.lumina."""
        native = tmp_path / ".lumina"
    ...
    def test_hermes_home_is_docker(self, tmp_path, monkeypatch):
        """When HERMES_HOME points outside ~/.lumina (Docker), returns HERMES_HOME."""
    ...
    def test_hermes_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any HERMES_HOME outside ~/.lumina is treated as the root."""
    ...
    def test_no_hermes_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\lumina, not ~/.lumina."""
        ...
        assert get_default_hermes_root() == local_appdata / "lumina"

    def test_no_hermes_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/lumina without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("HERMES_HOME", raising=False)
        ...
        assert get_default_hermes_root() == home / "AppData" / "Local" / "lumina"
...
class TestGetHermesHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When HERMES_HOME is unset on Windows, use %LOCALAPPDATA%\\lumina."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        ...
        assert get_hermes_home() == local_appdata / "lumina"
...
class TestGetProcessHermesHome:
    ...
    def test_env_unset_returns_platform_default(self, tmp_path, monkeypatch):
        monkeypatch.delenv("HERMES_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        assert get_process_hermes_home() == tmp_path / ".lumina"
