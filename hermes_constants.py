...
def get_default_hermes_root() -> Path:
    ...
    (``~/.lumina`` on POSIX, ``%LOCALAPPDATA%\\lumina`` on native Windows).

    In Docker or custom deployments where ``HERMES_HOME`` points outside
    ``~/.lumina`` (e.g. ``/opt/data``), returns ``HERMES_HOME`` directly
    — that IS the root.

    In profile mode where ``HERMES_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.lumina/profiles/coder``) and Docker
    ...
    try:
        ...
        # HERMES_HOME is under ~/.lumina (normal or profile mode)
        return native_home
    except ValueError:
        pass
...
def display_hermes_home() -> str:
    ...
    Uses ``~/`` shorthand for readability::
        ...
        default:  ``~/.lumina``
        profile:  ``~/.lumina/profiles/coder``
        custom:   ``/opt/hermes-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.lumina``.  For code that needs a real ``Path``, use
