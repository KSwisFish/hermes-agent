...
If you prefer a real POSIX environment (for the dashboard's embedded terminal, `fork` semantics, Linux-style file watchers, etc.), see the **[Windows (WSL2) Guide](./windows-wsl-quickstart.md)**. Both coexist cleanly: native data lives under `%LOCALAPPDATA%\lumina`, WSL data lives under `~/.lumina`.
:::
...
No admin rights required. The installer goes to `%LOCALAPPDATA%\lumina\` and adds `hermes` to your **User PATH** — open a new terminal after it finishes.
...
| `-HermesHome` | `%LOCALAPPDATA%\lumina` | Override data directory |
| `-InstallDir` | `%LOCALAPPDATA%\lumina\lumina-agent` | Override code location |
...
3. **Installs Node.js 22** (winget if available, else a portable Node tarball unpacked under `%LOCALAPPDATA%\lumina\node`). Used for the browser tool and the WhatsApp bridge.
4. **Installs portable Git** — if `git` is already on PATH the installer uses it; otherwise it downloads a trimmed, self-contained **PortableGit** (~45 MB, from the official `git-for-windows` release) to `%LOCALAPPDATA%\lumina\git`. No admin, no Windows installer registry, no interference with anything else on the box.
5. **Clones the repo** to `%LOCALAPPDATA%\lumina\lumina-agent` and creates a virtualenv inside it.
...
9. **Adds `%LOCALAPPDATA%\lumina\lumina-agent\venv\Scripts` to User PATH and sets `HERMES_HOME=%LOCALAPPDATA%\lumina`** — exposes the `hermes` command (and points it at your data dir) after you open a new terminal.
...
2. `%LOCALAPPDATA%\lumina\git\usr\bin\bash.exe` (installer-managed PortableGit).
3. `%LOCALAPPDATA%\lumina\git\bin\bash.exe` (older Git-for-Windows layout).
...
| Path | Contents |
|---|---|
| `%LOCALAPPDATA%\lumina\lumina-agent\` | Git checkout + venv. `venv\Scripts\hermes.exe` is the command added to User PATH. Safe to `Remove-Item -Recurse` and reinstall. |
| `%LOCALAPPDATA%\lumina\git\` | PortableGit (only if the installer provisioned it). |
| `%LOCALAPPDATA%\lumina\node\` | Portable Node.js (only if the installer provisioned it). |
| `%LOCALAPPDATA%\lumina\bin\` | Hermes's managed `uv.exe` (the Python manager it uses for updates). |
| `%LOCALAPPDATA%\lumina\` (root) | Your config, auth, skills, sessions, logs (`config.yaml`, `.env`, `skills\`, `sessions\`, `logs\`, …). **Survives reinstalls.** |

On native Windows the installer sets `HERMES_HOME=%LOCALAPPDATA%\lumina`, so your data and the disposable install live under the **same** `%LOCALAPPDATA%\lumina` root: the install/runtime is the `lumina-agent\`, `git\`, `node\`, and `bin\` subdirectories, while your data files sit directly in `%LOCALAPPDATA%\lumina`. Reinstalling only replaces the `lumina-agent\` checkout, so your data survives — but because the two share a root, **don't** `Remove-Item -Recurse %LOCALAPPDATA%\lumina` if you want to keep your data; delete the `lumina-agent\` subdirectory instead. Your data directory is identical in shape to a Linux `~/.lumina`, so you can mirror it between machines.

**Override `HERMES_HOME`:** set the environment variable to point at a different data dir (e.g. `%USERPROFILE%\.lumina` to match a Linux/WSL layout). Works the same as on Linux.
...
The installer adds `%LOCALAPPDATA%\lumina\lumina-agent\venv\Scripts` to your **User PATH** via `[Environment]::SetEnvironmentVariable`. Existing terminals don't pick this up — open a new PowerShell window (or Windows Terminal tab) after installation. Close-and-reopen, don't `$env:PATH += …` by hand unless you know what you're doing.

Verify:

```powershell
Get-Command hermes        # should print C:\Users\<you>\AppData\Local\lumina\lumina-agent\venv\Scripts\hermes.exe
hermes --version
```

### Environment variables

Hermes honors both `$env:X` (process-scope) and User environment variables (permanent, set in System Properties → Environment Variables). Setting API keys in `%LOCALAPPDATA%\lumina\.env` (your `HERMES_HOME`) is the normal path — same as Linux:
...
That's the clean path — removes the schtasks entry, Startup folder shortcut, `hermes.cmd` shim, deletes `%LOCALAPPDATA%\lumina\lumina-agent\`, and trims the User PATH. It leaves the rest of `%LOCALAPPDATA%\lumina\` alone (your config, auth, skills, sessions, logs) in case you're reinstalling.

To nuke everything:

```powershell
hermes uninstall
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\lumina"
# Also remove a legacy CLI/WSL data dir if you ever used one:
Remove-Item -Recurse -Force "$env:USERPROFILE\.lumina"
```
...
Open a new PowerShell window. The installer added `%LOCALAPPDATA%\lumina\bin` to User PATH, but existing shells need to be restarted to pick it up. In the meantime you can run `& "$env:LOCALAPPDATA\lumina\bin\hermes.cmd"`.
